...
###################        RF       #######################################
# Dp ----------
fn <- Sys.glob(file.path(path, "RandomForest/*.tif"))
suit <- rast(fn)
crops <- sub(paste0(path, "/RandomForest/"), "", sub("_RFpred.tif", "", fn))
...
app(suit, fun = f, filename = file.path(path, "Dp_Maxent_MonfCat.tif"),
    nodes = 7, overwrite = T, 
    wopt = list(names = "Dp_Monf", filetype = "GTiff", progress = 1))


# Map plot --------
Dp <- rast(file.path(path, "Dp_Maxent_MonfCat.tif"))

ar = ncol(Dp)/nrow(Dp)

{ # run this line to save plot 
  fgfn = ('Maps/Dp_Maxent_MonfCat.tif')
  tiff(filename = fgfn, units = "in",
       width = ncol(Dp)/300, 
       height = (ncol(Dp)/300)/ar, 
       type = "cairo", res = 300, 
       compression = "zip")
  {
    ...
    plot(Dp, axes = FALSE, col = cols,
         maxcell = ncell(Dp), 
         mar = c(2,0,2,4), main = "'Potential' Diversity (Maxent - Monfreda)")
    plot(ne_countries(), lwd = 1, add = T)
  }
  dev.off()
}
...
for(i in 1:length(crops)){
  { # run this line to save plot 
    ci <- grep(paste0('^',crops[i],"_"), names(suit))
    fgfn = file.path(smdir, paste0(crops[i], '.tif'))
    tiff(filename = fgfn, units = "in",
         width = ncol(suit[[ci]])/300, 
         height = (ncol(suit[[ci]])/300)/ar, 
