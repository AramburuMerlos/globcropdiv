...
fig.file = "G:/My Drive/globcropdiv/Plots/aD_fx_cD.png"
# Delete file if it exists
if(file.exists(fig.file)) file.remove(fig.file)
png(filename = fig.file, units = 'in', width = 3.3, 
    height = 3, type = "cairo", res = 300, pointsize = 11)

par(mgp = c(0,0.9,0), oma = c(3,3,2,0), mar = c(.8,0.2,.2,0.2))
layout(t(1:2), width = c(7,2), height = c(1,1))

xr <- yr <- c(1, 50)

plot_frame(xr, yr, 
           xlabel = bquote(italic(cD)), 
           ylabel = bquote(italic(aD)))

ad_plot(d, "cD", "aD", cD_brks, aD_brks, add = T, ibrks = ibrks, add_loess = F, showmax = T)
legend_image <- as.raster(matrix(viridis::mako(513), ncol=1))
plot(c(0,2),c(0,1),type = 'n', axes = F,xlab = '', ylab = '') 
text(x = 0.7, y = 0.95, 'Mha', cex = 1.1, adj = 0.5)
rasterImage(legend_image, 0, 0.1, 1, .85)
